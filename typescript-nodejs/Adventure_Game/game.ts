...
async function game() {
  ...
  console.log(
    chalk.bold.bgYellow.green.bold.bold("\n\t# WELCOME TO THE DUNGEON! #")
  );
  ...
  GAME: while (running) {
    ...
    while (enemyHealth > 0) {
      ...
      if (userquery.game_options === "Attack") {
        ...
        if (numHealthPotions > 0) {
          ...
          console.log(
            chalk.yellow.bold(
              `You don't have any health postion left. Defeat enemy for a chance to get one!`
            )
          );
        }
    ...
    if (health < 1) {
      console.log(
        chalk.red.bold(`\t> You limp out of the dungeon, too weak for battle`)
      );
      break;
    }
    console.log("------------------------------------------------------------");
  ...
  console.log(
    chalk.bgCyanBright.rgb(0, 0, 255).bold(`\n\tTHANKS FOR PLAYING!\n`)
  );
}
