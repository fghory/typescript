...
async function exchange() {
    console.log(chalk.bgRgb(0, 0, 255).yellow.bold("\nCONSOLE CURRENCY EXCHANGE\n"));
    await inquirer
        .prompt([
        {
            type: "list",
        ...
        console.log(`Curent Exchange Rate: ${chalk.yellow.bold(`${currencyfrom}`)}  = ${chalk.greenBright.bold(`${exchangerate.toFixed(2)} ${currencyto}`)} `);
        console.log(`You will get ${chalk.greenBright.bold(`${finalvalue.toFixed(2)} ${currencyto}`)} for ${chalk.yellow.bold(`${amount} ${currencyfrom}`)}`);
    });
}
exchange();
