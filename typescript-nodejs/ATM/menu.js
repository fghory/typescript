...
export async function atmMenu(useres) {
        ...
        if (ans.atmmenu === "Balance Inquiry") {
            ...
            inquirer
                ...
                if (np.new_pin.length < 4 || np.new_pin.length > 4) {
                    console.log("You entered wrong userid or wrong value of new pin.");
                }
                else {
                    useres.userpin = np.new_pin;
                    console.log(`Your Pin Code has been changed to ${useres.userpin}`);
                }
            });
        }
            ...
            inquirer
                ...
                .then((amount) => {
                let amt = Number(amount.withdrawl);
            ...
            inquirer
                ...
                .then((amount) => {
                let amt = Number(amount.fastcash);
                let remaining_balance = useres.balance - amt;
                console.log(`Your remaining balance is Rs.${remaining_balance}`);
            });
        }
