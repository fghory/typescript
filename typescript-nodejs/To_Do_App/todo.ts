import { TASK, task } from "./todoData.js";
      ...
      if (selection.todoList === "View Tasks") {
          ...
          .then((detail) => {
            ...
            for (let j = 0; j < task.length; j++) {
              if (detail.showtask === task[j].taskname) {
                console.log(
                  `${chalk.bold.yellowBright("Task:")} ${task[j].taskname}`
                );
                console.log(
                  `${chalk.bold.yellowBright("Details:")} ${task[j].details}`
                );
                console.log(
                  `${chalk.bold.yellowBright("Due on:")} ${task[j].duedate}`
                );
                console.log(
                  `${chalk.bold.yellowBright("Status:")} ${task[j].state}`
                );
              }
            }
          });
        ...
        inquirer
          ...
          .then((new_task) => {
            task.push(new_task);
            console.log(
              `${chalk.bold.yellowBright(`${new_task.taskname}`)} added`
            );
          });
        ...
        inquirer
          ...
          .then((ans) => {
            ...
            console.log(
              `Task ${ans.deleteTask} deleted successfully. Updated list: `
            );
            task.map((item) => {
              ...
              if (updtask.updatetask === task[i].taskname) {
                inquirer
                  ...
                  .then((updt) => {
                    ...
                    console.log(
                      `${chalk.bold.yellowBright("Task:")} ${task[i].taskname}`
                    );
                    console.log(
                      `${chalk.bold.yellowBright("Details:")} ${
                        task[i].details
                      }`
                    );
                    console.log(
                      `${chalk.bold.yellowBright("Due on:")} ${task[i].duedate}`
                    );
                    console.log(
                      `${chalk.bold.yellowBright("Status:")} ${task[i].state}`
                    );
                  });
              }
            }
